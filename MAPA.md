...
- **`victoria-llm-gateway`** (servicio systemd): FastAPI en `:8010` (**loopback-only desde 2026-08-14**; GATEWAY_HOST=127.0.0.1) con autenticación por API keys (auth por NOMBRE de key — la key activa se llama `demo`; validado 2026-08-13). Código `/home/victoria/llm-gateway.py`, User=victoria, WorkingDirectory=/home/victoria. Env: `VLLM_URL` (default localhost:8000), `GATEWAY_PORT` (default 8010), `ADMIN_PASS` (login del panel). Rutas: `/v1/chat/completions`, `/v1/completions`, `/models`, `/admin*` (panel: llaves, dashboard, usage — **solo LAN**). **Acceso LAN sin puertos**: nginx TLS en victoria (443, cert mkcert para victoria.local) → proxy a 127.0.0.1:8010. URLs: **`https://victoria.local/admin`** (panel), **`https://victoria.local/v1/...`** (API). El túnel victoria.armada.do conecta al loopback 8010 (sin cambios). Health: `curl http://127.0.0.1:8010/v1/chat/completions` sin bearer → 401 `missing_api_key` = OK. ⚠️ `/v1/models` NO existe (404) — usar `/models`. Cert/nginx: `/etc/ssl/local-certs/victoria.local.{pem,key}`, `/etc/nginx/sites-available/gateway.conf`. CA mkcert de kalimete confiable en kalimete; en Windows instalar `~/rootCA-kalimete-victoria-local.crt` (kalimete).
