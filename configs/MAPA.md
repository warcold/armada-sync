...
| **rootsource** | 10.0.0.5 | puerto 31337 | `rootsource` (admin) | Servidor Spark: GPU GB10, gateway LLM de Victoria |

- DNS local: mDNS/avahi (`.local`). Resolven `kalimete.local`, `victoria.local`, `jonas.local`, `rootsource.local`.
- Firewall por host: UFW — rootsource y victoria SIN reglas activas (verificado 2026-08-12; revisar host por host antes de asumir).
...
- **Stack LLM reemplazado**: El router anterior (`baseline-router` :4000) fue reemplazado por:
  ...
  - **`openshell` sandbox** (contenedor Docker): OpenClaw gateway corriendo dentro de un sandbox NemoClaw en red `openshell-docker` (172.18.0.2). El gateway escucha en `:18789` dentro del sandbox.
  - **`nemoclaw-vllm`** y **`openshell`** se crearon automáticamente por NemoClaw/OpenShell — NO hay docker-compose files manuales.
...
### victoria (10.0.0.64)
- **Docker (2)**: victoria-voice (healthy), ollama (:11434, 2 semanas up)
- **Systemd propios**: ssh, nginx (⚠️ cert homeassistant sin permisos — pendiente), docker, lightdm, haveged, unattended-upgrades
- **Agentes opencode**: LIMPIADO 2026-08-12 — los 13 agentes viejos (cf-*, cloudflare, ecosistema, eco-*, *-ro) movidos a `~/.config/opencode/agent-backup-2026-08-12/` (validado: contenido idéntico al backup de kalimete). Victoria tiene 0 agentes activos.

### rootsource (10.0.0.5)
- **Docker (2)**: nemoclaw-vllm (:8000, vLLM Qwen3.6-35B-A3B-NVFP4), openshell-my-assistant (sandbox OpenClaw, :18789)
- **Systemd propios**: ssh, docker, containerd, cloudflared, llmgate (:4010), gnome-remote-desktop (:3389 RDP headless), gdm, dgx-dashboard, nvidia-persistenced, lldpd, smartmontools
- **GPU**: NVIDIA GB10 (~128 GB VRAM, ~47.7 GiB usados por vLLM, 0% util idle)
- **Directorio llmgate**: /home/rootsource/llmgate/ (llmgate.py, config.env, data/router.db, AGENTS.md)
...
## ⚠️ Regla anti-desborde de contexto (2026-08-12)

El modelo `nvidia/Qwen3.6-35B-A3B-NVFP4` tiene **máximo 163840 tokens** (no 262k). `opencode.jsonc` ahora usa `limit.context: 158000` y `output: 24000` (margen de seguridad). **NUNCA volcar archivos grandes al chat** (find/grep sobre node_modules, logs completos): usar `head`, `grep -c`, `wc -l` o escribir a /tmp y leer con offset. Síntoma de violación: `Type validation failed ... maximum context length is 163840 tokens`.
